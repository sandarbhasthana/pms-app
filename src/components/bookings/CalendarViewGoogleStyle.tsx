...
interface CalendarViewGoogleStyleProps {
...
export default function CalendarViewGoogleStyle({
  resources,
  eventSources,
  handleEventClick,
  handleDateClick,
  ...
  // State to track if user is dragging an event (to prevent swipe during drag)
  const [isDragging, setIsDragging] = React.useState(false);

  // Motion values for smooth gesture-based animation
  ...
  // Ref to track if we're currently in a gesture
  const isGesturing = useRef(false);
  const containerRef = useRef<HTMLDivElement>(null);

  // Track accumulated offset for continuous scrolling
  const accumulatedOffset = useRef(0);
  const lastProcessedOffset = useRef(0);
  ...
  // Track animation reference
  const animationRef = useRef<ReturnType<typeof animate> | null>(null);

  // Configure gesture handlers for continuous scrolling
  const bind = useDrag(
    ({
      movement: [mx],
      cancel,
      ...
      active,
      event,
      memo = accumulatedOffset.current
    }) => {
      ...
      // Prevent swipe during event drag or when disabled
      if (disableSwipe || isDragging) {
        cancel();
        return;
      }
      ...
      if (active) {
        ...
        // Get day width for calculating when to change dates
        const slotEl = containerRef.current?.querySelector(
          ".fc-timeline-slot"
        ) as HTMLElement;
        const dayWidth = slotEl?.clientWidth ?? 100;

        // Calculate total offset including accumulated from previous gestures
        const totalOffset = memo + mx;

        // Check if we've crossed a day boundary
        const daysCrossed = Math.floor(totalOffset / dayWidth);
        const remainder = totalOffset % dayWidth;

        // If we've crossed day boundaries, trigger date changes immediately
        if (daysCrossed !== 0) {
          const daysToMove = daysCrossed - lastProcessedOffset.current;

          if (daysToMove !== 0) {
            const api = calendarRef.current?.getApi();
            if (api) {
              const currentDate = api.getDate();
              const newDate = addDays(currentDate, -daysToMove); // Negative because right swipe = backward in time
              api.gotoDate(newDate);
              onDateChange(format(newDate, "yyyy-MM-dd"));
            }

            lastProcessedOffset.current = daysCrossed;
          }
        }

        // Set visual position to the remainder (keeps calendar centered)
        x.set(remainder);

        return memo;
      } else if (last) {
        // On release: just reset position smoothly
        ...
        // Reset accumulated offset and processed offset
        accumulatedOffset.current = 0;
        lastProcessedOffset.current = 0;
  ...
  // Enhanced event drop handler to track dragging state
  const handleEventDropWithDragTracking = React.useCallback(
    (arg: EventDropArg) => {
      setIsDragging(false);
      handleEventDrop(arg);
    },
  ...
  // Apply transform to ONLY the scrollable timeline content (not resource sidebar)
  React.useEffect(() => {
    if (containerRef.current && isMobile) {
      const unsubscribe = xSpring.on("change", (latest) => {
        if (!containerRef.current) return;

        // Transform ONLY the timeline scroller containers (NOT the resource sidebar)
        const timelineScrollers = containerRef.current.querySelectorAll(
          ".fc-timeline-body, .fc-timeline-header-row"
        );

        // Apply transform to all timeline scroller containers
        timelineScrollers.forEach((element) => {
          (element as HTMLElement).style.transform = `translateX(${latest}px)`;
        });
      });
  ...
  // Get bind handlers
  const bindHandlers = isMobile ? bind() : {};

  return (
    <div
      ref={containerRef}
      className="google-style-calendar-container"
      style={{
        touchAction: "pan-y", // Allow vertical scroll, prevent horizontal scroll
        position: "relative"
      }}
      onTouchStart={bindHandlers.onTouchStart}
      onTouchMove={bindHandlers.onTouchMove}
      onTouchEnd={bindHandlers.onTouchEnd}
      onTouchCancel={bindHandlers.onTouchCancel}
    >
      <FullCalendar
        ref={calendarRef}
        ...
        eventDrop={handleEventDropWithDragTracking}
        ...
        resourceAreaHeaderContent={
          <div className="pl-2">Room Types & Rooms</div>
        }
