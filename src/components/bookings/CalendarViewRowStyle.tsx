...
export default function CalendarViewRowStyle({
  ...
  const hasTriggeredSwipe = useRef(false); // Track if we've already triggered the date change

  // Detect if we're on client side and check mobile (prevent hydration mismatch)
  useEffect(() => {
    setMounted(true);
  ...
  const bind = useDrag(
    ({ movement: [mx], cancel, last, active, event, first }) => {
      // Only work on mobile/tablet and after mount
      if (!mounted || !isMobile) {
        cancel();
        return;
      }

      // Prevent swipe during event drag
      if (isDragging) {
        cancel();
        return;
      }
      ...
      if (active) {
        // Check if we've crossed the threshold during the swipe
        const threshold = 50; // Minimum swipe distance to trigger (50px)

        if (!hasTriggeredSwipe.current && Math.abs(mx) > threshold) {
          ...
          // Update calendar date with a slight delay to allow transition to start
          setTimeout(() => {
            const api = calendarRef.current?.getApi();
            if (api) {
        ...
        // On release: just reset the trigger flag
        hasTriggeredSwipe.current = false;
      }
    },
    {
      ...
      filterTaps: true,
      pointer: { touch: true },
      rubberband: false // Disable rubberband to prevent elastic bounce
    }
  );

  // No transform effect needed - we're updating calendar date directly instead

  // Apply transition effect to ONLY timeline area (not resource column)
  useEffect(() => {
    if (!mounted || !containerRef.current) return;

    const timelineElements = containerRef.current.querySelectorAll(
  ...
  // Make entire room type cell clickable (not just the arrow)
  useEffect(() => {
    if (!mounted || !containerRef.current) return;
    ...
    const handleCellClick = (event: MouseEvent) => {
      ...
      // Find the closest resource cell
      const resourceCell = target.closest(".fc-datagrid-cell.fc-resource");
      if (!resourceCell) return;

      // Check if this is a parent resource (room type) by looking for the expander
      const expander = resourceCell.querySelector(".fc-datagrid-expander");
      if (!expander) return;

      // Find the actual expander button
      const expanderButton = expander.querySelector(
        ".fc-icon-minus-square, .fc-icon-plus-square"
      ) as HTMLElement;
      if (!expanderButton) return;

      // If the click was not directly on the expander button, trigger it
      if (!target.closest(".fc-datagrid-expander")) {
        // Prevent default behavior to avoid highlighting
        event.preventDefault();
        event.stopPropagation();

        expanderButton.click();

        // Force remove any focus/active states
    ...
    // Add both click and mousedown listeners
  ...
  // Get bind handlers (only on client side to prevent hydration issues)
  const bindHandlers = mounted && isMobile ? bind() : {};

  return (
    <div ref={containerRef} {...bindHandlers} style={{ touchAction: "pan-y" }}>
      <FullCalendar
        ref={calendarRef}
