// File: src/app/dashboard/bookings-fixed/page.tsx
// FIXED VERSION - Optimized to prevent frequent refreshing and console spam
"use client";
...
import CalendarViewFixed from "@/components/bookings/CalendarViewFixed";
...
export default function BookingsCalendarPageFixed() {
  ...
  // FIXED: Stable today highlight that doesn't change every minute
  const todayDateString = useMemo(() => {
    const now = new Date();
    return now.toISOString().slice(0, 10); // YYYY-MM-DD format
  }, []); // Only compute once on mount

  // FIXED: Use stable date for today highlight
  ...
  // Debounce today updates to prevent frequent re-renders
  const debouncedToday = useDebounce(today, 5000); // 5 second debounce
  ...
  const eventSources = useMemo(() => {
    return [
      ...
      // Weekend/Sunday highlight function (unchanged)
      (
  ...
  useEffect(() => {
    ...
    // FIXED: Increased interval from 60s to 5 minutes to reduce re-renders
    const iv = setInterval(upd, 5 * 60 * 1000); // 5 minutes instead of 1 minute
